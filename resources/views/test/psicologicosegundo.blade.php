...
<div class="content-wrapper">
  <h1 align="center">Segundo reporte psicológico </h1>
  <!-- <a href="{{ url('user/create') }}"> -->
  <a href="javascript:void(0)">
    <button value="Alta" title="Alta segundo reporte psicologico" class="btn btn-success" id="registrar"><i class="fa fa-cloud-upload" aria-hidden="true"></i></button>
  </a>
  <a href="{{ url('pdfuser') }}">
  ...
  <table id="reportTable" class="table table-striped table-bordered" style="width:100%">
    <thead>
        ...
        <th>Pregunta once</th>
        <th>Pregunta doce</th>
        <th>Pregunta trece</th>
        <th>Pregunta catorce</th>
        <th>Pregunta quince</th>
        <th>Pregunta dieciseis</th>
        <th>Pregunta diecisiete</th>
        <th>Pregunta dieciocho</th>
        <th>Pregunta diecinueve</th>
        <th>Pregunta Veinte</th>
        <th>Acciones</th>
    </thead>
    <tbody>
      ...
      <tr>
        ...
        <td>{{ $test->pregunta_once }}</td>
        <td>{{ $test->pregunta_doce }}</td>
        <td>{{ $test->pregunta_trece }}</td>
        <td>{{ $test->pregunta_catorce }}</td>
        <td>{{ $test->pregunta_quince }}</td>
        <td>{{ $test->pregunta_dieciseis }}</td>
        <td>{{ $test->pregunta_diecisiete }}</td>
        <td>{{ $test->pregunta_dieciocho }}</td>
        <td>{{ $test->pregunta_diecinueve }}</td>
        <td>{{ $test->pregunta_veinte }}</td>
        <td>
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Reporte segundo de test</h5>
